...
for subjidx = 1:length(subjvec)
    ...
    myNLL = @(pars) mymodelHeuristicDDM_twobounds(pars, datasubj);
    
    init         = NaN(numinit, 9);
    init(:,1)    = log(rand(numinit,1)); %beta softmax
    init(:,2:5)  = randi(25, numinit,4) - 1; %4 criteria
    init(:,6:9)  = randi(25, numinit,4) - 1;
    
    lowK = 0;
    highK = inf;
    lowLimits = [-inf lowK lowK lowK lowK lowK lowK lowK lowK];
    highLimits = [inf highK highK highK highK highK highK highK highK];
...
save estimates_DDM_twobounds pars_est allbestNLL
